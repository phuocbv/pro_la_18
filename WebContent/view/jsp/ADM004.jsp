...
<body>
				...
				<tr>
					...
					<c:choose>
						<c:when test="${userId != null}">
							<input type="hidden" value="${Constant.TYPE_ADM005 }" name="type" />
							<input type="hidden" value="${userId}" name="userId" />
							<td><input class="btn" type="submit" value="編集" /></td>
							<td><input class="btn" type="button" value="削除"
								onclick="deleteUser()" /></td>
							<td><input class="btn btn_large" type="button" value="Change Password"
	...
	<c:if test="${userId != null}">
		<form
			action="${pageContext.request.contextPath}${Constant.URL_DELETE_USER }"
